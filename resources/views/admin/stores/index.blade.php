    ...
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.stores.create') }}">
                {{ trans(tenant()->id.'/global.add') }} {{ trans(tenant()->id.'/cruds.store.title_singular') }}
            </a>
            @can('csv_import')
...
<div class="card">
    <div class="card-header">
        <div class="row align-items-center">
            <div class="col-sm-6">
                <h4>
                {{ trans(tenant()->id.'/cruds.store.title_singular') }} {{ trans(tenant()->id.'/global.list') }}
                </h4>
            </div>
            @can('store_create')
        ...
        <div class="table-responsive">
            <table class=" table table-borderless table-striped table-hover datatable datatable-Store">
                <thead>
                    <tr>
                        <th>
                            {{ trans(tenant()->id.'/cruds.store.fields.id') }}
                        </th>

                        <th>
                            {{ trans(tenant()->id.'/cruds.store.fields.name') }}
                        </th>
                    ...
                    @foreach($stores as $key => $store)
                        <tr data-entry-id="{{ $store->id }}">
                            ...
                            <td class="text-center">
                                ...
                                @can('store_delete')
                                    <form action="{{ route('admin.stores.destroy', $store->id) }}" method="POST" onsubmit="return confirm('{{ trans(tenant()->id.'/global.areYouSure') }}');" style="display: inline-block;">
