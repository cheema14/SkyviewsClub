...
<div class="card">
    <div class="card-header">
        <h4>
        {{ trans(tenant()->id.'/global.edit') }} {{ trans(tenant()->id.'/cruds.store.title_singular') }}
        </h4>
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.stores.update", [$store->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input maxlength="100" class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $store->name) }}" required>
                @if($errors->has('name'))
