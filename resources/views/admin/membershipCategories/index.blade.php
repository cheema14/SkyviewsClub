...
<!-- @can('membership_category_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.membership-categories.create') }}">
                {{ trans(tenant()->id.'/global.add') }} {{ trans(tenant()->id.'/cruds.membershipCategory.title_singular') }}
            </a>
            @can('csv_import')
...
<div class="card">
    <div class="card-header">
        <div class="row align-items-center">
            <div class="col-sm-6">
                <h4>
                {{ trans(tenant()->id.'/cruds.membershipCategory.title_singular') }} {{ trans(tenant()->id.'/global.list') }}
                </h4>
            </div>
            @can('membership_category_create')
            <div class="col-sm-6 ml-auto text-sm-right">
                <a class="btn btn-info px-4 mr-2" href="{{ route('admin.membership-categories.create') }}">
                {{ trans(tenant()->id.'/global.add') }} {{ trans(tenant()->id.'/cruds.membershipCategory.title_singular') }}
                </a>
                @can('csv_import')
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-borderless table-striped table-hover datatable datatable-MembershipCategory">
                <thead>
                    <tr>

                        <th>
                            {{ trans(tenant()->id.'/cruds.membershipCategory.fields.id') }}
                        </th>
                        <th>
                            {{ trans(tenant()->id.'/cruds.membershipCategory.fields.name') }}
                        </th>
                        ...
                        <tr data-entry-id="{{ $membershipCategory->id }}">
                            ...
                            <td class="text-center">
                                ...
                                @can('membership_category_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.membership-categories.edit', $membershipCategory->id) }}">
                                        {{ trans(tenant()->id.'/global.edit') }}
                                    </a>
                                @endcan

                                @can('membership_category_delete')
                                    <form action="{{ route('admin.membership-categories.destroy', $membershipCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans(tenant()->id.'/global.areYouSure') }}');" style="display: inline-block;">
...
@can('membership_category_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.membership-categories.massDestroy') }}",
