...
<!-- @can('order_create')
    <div style="margin-bottom: 10px;" class="row">
        @can('order_create')
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.list-floor-orders') }}">
        ...
        <table class=" table table-borderless table-striped table-hover ajaxTable datatable datatable-Order">
            <thead>
                <tr>
                    ...
                    {{-- <th>
                        {{ trans(tenant()->id.'/cruds.floors.title_singular') }}
                    </th> --}}
                    <th>
                        Actions
                    </th>
...
$(function () {
    ...
    @endcan
        ...
        let dtOverrideGlobals = {
            ...
            ajax: {
                url: "{{ route('admin.list-floor-orders') }}",
                type: "GET",
                data: function (d) {
                    d.status = status;
                },
    ...
    $(document).on("click","#search",function(){
        ...
        // Clear the DataTable
        table.clear().draw();
        table.ajax.url("{{ route('admin.list-floor-orders') }}?fromDate="+fromDate+"&toDate="+toDate+"&orderStatus="+selectedStatus).load();
    ...
    $(document).on("click","#reset",function(){
        reset_form_fields();
        table.clear().draw();
        table.ajax.url("{{ route('admin.list-floor-orders') }}").load();
    });
