...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.sportBillingItem.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.sport-billing-items.update", [$sportBillingItem->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('billing_division') ? 'is-invalid' : '' }}" name="billing_division_id" id="billing_division_id">
                    @foreach($billing_divisions as $id => $entry)
                        <option value="{{ $id }}" {{ (old('billing_division_id') ? old('billing_division_id') : $sportBillingItem->billing_division->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('billing_item_type') ? 'is-invalid' : '' }}" name="billing_item_type_id" id="billing_item_type_id">
                    @foreach($billing_item_types as $id => $entry)
                        <option value="{{ $id }}" {{ (old('billing_item_type_id') ? old('billing_item_type_id') : $sportBillingItem->billing_item_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('billing_item_class') ? 'is-invalid' : '' }}" name="billing_item_class_id" id="billing_item_class_id">
                    @foreach($billing_item_classes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('billing_item_class_id') ? old('billing_item_class_id') : $sportBillingItem->billing_item_class->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('billing_item_name') ? 'is-invalid' : '' }}" name="billing_item_name_id" id="billing_item_name_id">
                    @foreach($billing_item_names as $id => $entry)
                        <option value="{{ $id }}" {{ (old('billing_item_name_id') ? old('billing_item_name_id') : $sportBillingItem->billing_item_name->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('quantity') ? 'is-invalid' : '' }}" type="number" name="quantity" id="quantity" value="{{ old('quantity', $sportBillingItem->quantity) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('rate') ? 'is-invalid' : '' }}" type="number" name="rate" id="rate" value="{{ old('rate', $sportBillingItem->rate) }}" step="1" required>
                @if($errors->has('rate'))
                    <div class="invalid-feedback">
                        {{ $errors->first('rate') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number" name="amount" id="amount" value="{{ old('amount', $sportBillingItem->amount) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('billing_issue_item') ? 'is-invalid' : '' }}" name="billing_issue_item_id" id="billing_issue_item_id">
                    @foreach($billing_issue_items as $id => $entry)
                        <option value="{{ $id }}" {{ (old('billing_issue_item_id') ? old('billing_issue_item_id') : $sportBillingItem->billing_issue_item->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
