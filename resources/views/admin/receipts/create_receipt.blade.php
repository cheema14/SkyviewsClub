...
<div class="card">
    <div class="card-header">
        <h4>
        {{ trans(tenant()->id.'/global.create') }} {{ trans(tenant()->id.'/cruds.cash_receipts.title_cash_receipt') }}
        </h4>
    </div>
    ...
    @csrf
        <div class="card-body row" x-data="handler()">
            <div class="form-group col-md-4">
                <label class="" for="name">{{ trans(tenant()->id.'/cruds.cash_receipts.fields.receipt_no') }}</label>
            ...
            <div class="form-group col-md-4">
                <label class="" for="name">{{ trans(tenant()->id.'/cruds.cash_receipts.fields.receipt_date') }}</label>
            ...
            <div class="form-group col-md-4">
                <label class="" for="name">{{ trans(tenant()->id.'/cruds.cash_receipts.fields.club') }}</label>
            ...
            <div class="form-group col-md-4">
                <label class="" for="name">{{ trans(tenant()->id.'/cruds.order.fields.grand_total') }}</label>
                <input class="form-control" type="text" name="grand_total" id="grand_total" value="{{ $order->grand_total }}" disabled>
            </div>

            <input type="hidden" id="total_deals_amount" value="{{ $total_deals_amount }}" >
            
            {{-- No discount if there is a menu which has_discount = 1 --}}
            
                
                <div class="form-group col-md-4">
                    <label>{{ trans(tenant()->id.'/cruds.order.fields.discount') }}</label>
                ...
                <div class="form-group col-md-4">
                    <label class="" for="name">{{ trans(tenant()->id.'/cruds.order.fields.after_discount') }}</label>
    ...
    function handler() {
        return {
            ...
            calculateDiscountOnChange($event){
                ...
                let total_deals_amount = document.getElementById("total_deals_amount").value;
                let total_payable = document.getElementById("grand_total").value - total_deals_amount;
                // console.log("total_deals_amount",total_deals_amount);
                if(total_payable){
                    discount = (total_payable * $event.target.value) / 100;
                    document.getElementById("afterDiscountAmount").value =  (parseFloat(total_deals_amount) + total_payable - discount).toFixed(2);
