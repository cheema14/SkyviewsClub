...
<div class="card">
    <div class="card-header">
        <div class="row align-items-center">
            <div class="col-sm-6">
                <h4>
                    {{ trans(tenant()->id .'/cruds.designation.title_singular') }} {{ trans(tenant()->id .'/global.list') }}
                </h4>
            </div>
            ...
            <div class="col-sm-6 ml-auto text-sm-right">
                <a class="btn btn-info px-4 mr-2" href="{{ route('admin.designations.create') }}">
                    {{ trans(tenant()->id .'/global.add') }} {{ trans(tenant()->id .'/cruds.designation.title_singular') }}
                </a>
                @can('csv_import')
        ...
        <div class="table-responsive">
            <table class=" table table-borderless table-striped table-hover datatable datatable-Designation">
                <thead>
                    <tr>
                        <th>
                            {{ trans(tenant()->id .'/cruds.designation.fields.id') }}
                        </th>
                        <th>
                            {{ trans(tenant()->id .'/cruds.designation.fields.title') }}
                        </th>
                        ...
                        <tr data-entry-id="{{ $designation->id }}">
                            ...
                            <td class="text-center">
                                ...
                                @can('designation_delete')
                                    <form action="{{ route('admin.designations.destroy', $designation->id) }}" method="POST" onsubmit="return confirm('{{ trans(tenant()->id .'/global.areYouSure') }}');" style="display: inline-block;">
